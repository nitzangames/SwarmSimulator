...
[board.js](src/board.js) keeps one crowd mesh and switches between shared geometry/material pairs for **2 triangles** and **3D fish**. Each mode uses `InstancedBufferGeometry` and the same simulation attributes. This abbreviated example shows how a field becomes a GPU instance attribute:
...
The vertex shader places each selected shape and rotates it toward its velocity. Color variation is static. Dynamic attributes reference the existing state buffers, and reusable update-range records select only the live prefixes after the first upload.
...
### 5. Animate a real fish skeleton with shared data

[fish.js](src/fish.js) authors a small 3D fish with a rounded body, fins, and eyes. It creates a conventional Three.js `SkinnedMesh`, a hierarchy of five `Bone` objects, normalized `skinIndex`/`skinWeight` attributes, and a looping quaternion animation clip. The spine bends laterally, with increasing motion toward the tail. The fish model is generated by this repository's code; no external model download is required.

At startup, one `AnimationMixer` samples the rig into 64 poses. A 20 KiB `Float32Array` stores the five bone matrices per pose, including inverse-bind transforms, and becomes a shared `DataTexture`. The crowd shader reads adjacent poses, blends the two bone influences for each vertex, and applies each agent's heading and position. Fish use different animation phases and cycle rates derived from their existing appearance value. There is no skeleton or mixer allocated per crowd agent, and both appearance modes draw the school in one draw call.

The **Inspect fish** dialog uses that same authored `SkinnedMesh` and clip directly. Drag to rotate it and enable **Show skeleton** to see the bone chain and joints. This optional viewer has its own renderer; its draw calls are excluded from the crowd's counter. Opening it still adds work and can affect measured frame intervals.

Switching appearances retains flock positions, velocities, and settings. Automatic mode restarts tuning and caps the initial fish population at the configured starting count; manual mode preserves the selected count. Pause and movement speed also control swimming animation. Fish add vertices, skinning, and lighting work, so one draw call does not imply equal performance between modes. Motion remains in the original 2D simulation world; the fish themselves are 3D.
...
| Fish asset        | [src/fish.js](src/fish.js)                               | Mesh, skeleton, swim clip, baked pose texture, skinning shaders |
| Model inspector   | [src/fishInspector.js](src/fishInspector.js)             | Optional close-up viewer with skeleton overlay                  |
...
- **2 triangles / 3D fish** switches appearance; **Inspect fish** opens the rig viewer.
...
- Separation, alignment, cohesion, and movement speed remain adjustable in every preset.
...
- **Typed-array pool** measures simulation buffers, excluding fish asset/animation data, browser and GPU memory.
...
The Node tests cover deterministic motion, finite/bounded state, grid membership, edge wrapping, flocking forces, backing-buffer reuse, population adaptation, skin weights, baked poses against Three.js skinning, and the swim clip's deformation/loop. The browser check covers both appearances, the animated fish inspector, pause/resume, mouse/touch interaction, the explanation panel, the complete pool in triangle mode, and WebGL context recovery. Screenshots are written to `test-results/`.
...
- [Three.js SkinnedMesh](https://threejs.org/docs/pages/SkinnedMesh.html), [Skeleton](https://threejs.org/docs/pages/Skeleton.html), and [AnimationMixer](https://threejs.org/docs/pages/AnimationMixer.html).
